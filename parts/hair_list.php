...
for ($i=0;$i<$num;$i++){
    //$filenameにファイル名を設定
    $filename = "parts/hair/cv/" . $array[$i];
    $filename2 = "parts/hair/" . $array[$i];
    ...
    if  (
         Eregi('png$', $filename)) {
        print("<td><div><input type=image class=ct name=hair img src= " .$filename2. " onClick=bgImgChange('url($filename)','hairParts');></div></td>");
