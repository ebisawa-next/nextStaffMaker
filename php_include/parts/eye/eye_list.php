...
for ($i=0;$i<$num;$i++){
    //$filenameにファイル名を設定
    $filename = "images/parts/eye/" . $array[$i];
    $filename2 = "images/parts/eye/cv/" . $array[$i];
    ...
    if  (
         Eregi('png$', $filename)) {
        print("<td><div><input type=image name=eye img src= " .$filename. " onClick=bgImgChange('url($filename2)','eyeParts');></div></td>");
...
?>
<td><div><input type="image" name="mayu" onClick="bgImgChange('url(images/none.png)','eyeParts');" img src= "images/none_s.png"></div></td></tr></table>