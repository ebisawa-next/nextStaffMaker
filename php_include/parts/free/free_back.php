...
for ($i=0;$i<$num;$i++){
    //$filenameにファイル名を設定
    $filename = "images/parts/free/back/" . $array[$i];
    $filename = "images/parts/free/back/cv/" . $array[$i];
    ...
    if  (
         Eregi('png$', $filename)) {
        print("<td><div><input type=image name=ace img src= " .$filename. " onClick=bgImgChange('url($filename2)','backOtherParts');></div></td>");
...
<td><div><input type="image" name="mayu" onClick="bgImgChange('url(images/none.png)','backOtherParts');" img src= "images/none_s.png"></div></td></tr></table>
</div>