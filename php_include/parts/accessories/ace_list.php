...
for ($i=0;$i<$num;$i++){
    //$filenameにファイル名を設定
    $filename = "images/parts/cloth/" . $array[$i];
    ...
    if (Eregi('png$', $filename)) {
        print("<td><div><input type=image name=body img src= " .$filename. " onClick=bgImgChange('url($filename)','clothParts');></div></td>");
